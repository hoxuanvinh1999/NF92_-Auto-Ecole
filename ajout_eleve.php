...
<body>
  ...
  if (is_numeric($annee) and is_numeric($mois) and is_numeric($jour)) {
    $date_valide = checkdate($jour,$mois,$annee);
  } else {
    $date_valide = False;
  }
  ...
  if (empty($nom) or empty($prenom) or is_numeric($nom) or is_numeric($prenom) or ($date_today<$date_birth) or ($date_birth==false) or ($date_valide==false)) {
    echo"<br> <span> Recommencer, il y a une error <span> <br>";
    ...
    if (!$result) {
      ...
      if ($unknow=mysqli_fetch_array($result,MYSQL_NUM)) {
        ...
        echo "<div class='row'>";
          echo "<form action='ajoute_eleve.html' method='post'>";
     ...
     echo "</div>";
        ...
        $query= $query = "INSERT INTO eleves VALUES (NULL, '$nom', '$prenom', '$date_naissance', '$today');";
        echo "<br>$query<br>";
        $result = mysqli_query($connect, $query);
        if (!$result) {
